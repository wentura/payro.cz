 ...
 * Edit Invoice Page
 *
 * Form for editing a draft invoice with items
 */
...
export default function EditInvoicePage({ params }) {
  ...
  const fetchInvoiceData = async () => {
    try {
      ...
      if (!response.ok || !result.success) {
        setError("Faktura nenalezena nebo nelze upravovat");
        setIsLoading(false);
        return;
      }
      ...
      if (invoice.status_id !== 1) {
        setError("Lze upravovat pouze faktury ve stavu Koncept");
        setIsLoading(false);
        return;
      }
  ...
  const fetchDropdownData = async () => {
    try {
      const [clientsRes, dueTermsRes, paymentTypesRes, unitsRes] =
        await Promise.all([
          fetch("/api/clients"),
          fetch("/api/due-terms"),
      ...
      const [clientsData, dueTermsData, paymentTypesData, unitsData] =
        await Promise.all([
          clientsRes.json(),
          dueTermsRes.json(),
          paymentTypesRes.json(),
          unitsRes.json(),
        ]);

      if (clientsData.success) setClients(clientsData.data || []);
      if (dueTermsData.success) setDueTerms(dueTermsData.data || []);
